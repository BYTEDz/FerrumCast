...
// Zero-latency critical: force UDP queue buffer to 1 so frames dispatch to network instantly
fn default_queue_max_buffers() -> u32 { 1 }
...
impl ConfigStore {
    ...
    pub fn new_from_args() -> Self {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--monitor-index" => {
                    if let Some(val) = args.next() {
                        if let Ok(v) = val.parse() { cfg.monitor_index = v; }
                    }
                }
                "--speed-preset" => { if let Some(val) = args.next() { cfg.speed_preset = val; } }
                "--tune" => { if let Some(val) = args.next() { cfg.tune = val; } }
                "--nvenc-preset" => { if let Some(val) = args.next() { cfg.nvenc_preset = val; } }
                "--nvenc-tune" => { if let Some(val) = args.next() { cfg.nvenc_tune = val; } }
                "--rc-mode" => { if let Some(val) = args.next() { cfg.rc_mode = val; } }
                "--key-int-max" => {
                    if let Some(val) = args.next() {
                        if let Ok(v) = val.parse() { cfg.key_int_max = v; }
                    }
                }
                "--srtp-key" => { if let Some(val) = args.next() { cfg.srtp_key = Some(val); } }
                _ => {}
            }
        }
        info!(
            "Pre-seeded config from args: encoder={:?} bitrate={} width={:?} height={:?} fps={:?} \
            gdi={} monitor_index={} rc_mode={} key_int_max={}",
            cfg.encoder, cfg.bitrate, cfg.width, cfg.height, cfg.framerate,
            cfg.gdi, cfg.monitor_index, cfg.rc_mode, cfg.key_int_max
        );
        Self(RwLock::new(cfg))
    }
    ...
    pub fn set(&self, cfg: StreamConfig) {
        ...
        *guard = cfg;
        info!(
            "Stream config updated: bitrate={}kbps encoder={:?} rc_mode={} monitor_index={}",
            guard.bitrate, guard.encoder, guard.rc_mode, guard.monitor_index
        );
    }
}